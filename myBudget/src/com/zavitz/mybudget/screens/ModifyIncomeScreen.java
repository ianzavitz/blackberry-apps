...
public class ModifyIncomeScreen extends PopupScreen {

	private ButtonField ok, close;
	private AutoTextEditField amount;
	private ObjectChoiceField type;

	public ModifyIncomeScreen() {
		super(new VerticalFieldManager());

		LabelField header = new LabelField("Modify Monthly Income");
		header.setFont(getFont().derive(Font.BOLD));
		LabelField sheader = new LabelField("Currently: $" + Utilities.formatDouble(UiApp.activeManager.getIncome()));
		...
		type.setChangeListener(new FieldChangeListener() {
			public void fieldChanged(Field f, int instance) {
				...
				case 1:
					break;
				case 2:
					amount.setText(String.valueOf(Utilities.formatDoubleNoCommas(UiApp.activeManager
							.getIncome())));
					break;
				}
			}
		...
		ok.setChangeListener(new FieldChangeListener() {
			public void fieldChanged(Field f, int instance) {
				switch (type.getSelectedIndex()) {
				case 0:
					UiApp.activeManager.setIncome(UiApp.activeManager
							.getIncome()
				...
				case 1:
					...
					break;
				case 2:
					UiApp.activeManager.setIncome(Double.parseDouble(amount.getText()));
					break;
				}
				UiApp.save();
				setDirty(false);
